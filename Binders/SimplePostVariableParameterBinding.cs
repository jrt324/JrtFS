...
public class SimplePostVariableParameterBinding : HttpParameterBinding
{
    ...
    object StringToType(string stringValue)
    {
        ...
        {
            return string.IsNullOrWhiteSpace(stringValue) ? null : stringValue.ConvertType(Descriptor.ParameterType, stringValue);
        }
        else if (Descriptor.ParameterType!=typeof(byte[]) && Descriptor.ParameterType.IsArray)
        {
            ...
            {
                stringValue = "[" + stringValue.Split(new char[]{','},StringSplitOptions.RemoveEmptyEntries).Join(",","\"") + "]";
            }
